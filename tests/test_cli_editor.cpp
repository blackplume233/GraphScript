...
TEST(CLIEditor, SourceBackedGraphCommandsPatchTextWithoutReemit) {
    ...
    EXPECT_TRUE(editor.last_command_succeeded());
    EXPECT_TRUE(editor.execute("set_init_ctor extra asset SoftObjectPath AssetPath"));
    EXPECT_TRUE(editor.last_command_succeeded());
    EXPECT_TRUE(editor.execute("set_init_ctor_arg extra asset OtherAsset"));
    EXPECT_TRUE(editor.last_command_succeeded());
    EXPECT_TRUE(editor.execute("set_init_ctor_type extra asset AssetRef"));
    EXPECT_TRUE(editor.last_command_succeeded());
    EXPECT_TRUE(editor.execute("rename_init extra asset resource"));
    EXPECT_TRUE(editor.last_command_succeeded());
    EXPECT_TRUE(editor.execute("set_init extra temp keep"));
    EXPECT_TRUE(editor.last_command_succeeded());
    EXPECT_TRUE(editor.execute("unset_init extra temp"));
    EXPECT_TRUE(editor.last_command_succeeded());
    EXPECT_TRUE(editor.execute("set_init_expr extra Factory(seed)"));
    EXPECT_FALSE(editor.last_command_succeeded());
    EXPECT_TRUE(editor.execute("event OnStart"));
    EXPECT_TRUE(editor.last_command_succeeded());
    EXPECT_TRUE(editor.execute("flow logger.exit wait.enter"));
    ...
    EXPECT_NE(emitted.find("node extra {\n        type PrintString;\n        message: inserted;\n        resource: AssetRef(OtherAsset);"), std::string::npos);
    EXPECT_EQ(emitted.find("temp: keep;"), std::string::npos);
    EXPECT_EQ(emitted.find("Factory(seed)"), std::string::npos);
