...
def main():
    ...
    try:
        with sync_playwright() as p:
            ...
            page.route("**/api/emit", lambda route: route.fulfill(
                ...
                body='import "tests\\\\fixtures\\\\mixed_declarations.d.gs";\n\ngraph ImportBaseDir {\n    event OnStart {\n    }\n}\n',
            ))
            ...
            page.wait_for_function("() => document.body.innerText.includes('ImportBaseDir')", timeout=10000)
    ...
    expected_base = normalized(REPO)
    actual_base = normalized(diagnostics_requests[-1].get("base_dir", "")) if diagnostics_requests else ""
    print("\nSource import base dir regression")
    print(f"  expected: {expected_base}")
    print(f"  actual:   {actual_base}")
    if actual_base != expected_base:
        raise SystemExit("Source diagnostics should resolve relative imports from the session root")
