...
- CUE：JSON superset + constraint language，可作为 GraphScript 声明、默认值、约束、unification 冲突诊断的设计参考；不直接解决 source ownership 和图编辑回写策略。参考：https://cuelang.org/docs/
- JSON5：面向人类编辑的 JSON 扩展，可作为 GraphScript JSON-family 作者格式的低语义负担候选。参考：https://json5.org/；独立讨论见 [JSON5 图资产格式讨论稿](../plans/json5-graph-asset-format-discussion.md)。
